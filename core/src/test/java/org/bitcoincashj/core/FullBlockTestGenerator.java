...
public class FullBlockTestGenerator {
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        NewBlock b15 = createNextBlock(b13, chainHeadHeight + 6, out5, null);
        {
            ...
            byte[] outputScript = new byte[params.getMaxBlockSigops() - sigOps];
            ...
            checkState(sigOps == params.getMaxBlockSigops());
        }
        b15.solve();
        ...
        NewBlock b16 = createNextBlock(b15, chainHeadHeight + 7, out6, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSigops() - sigOps + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
            ...
            for (Transaction tx2 : b16.block.getTransactions())
                sigOps += tx2.getSigOpCount();
            checkState(sigOps == params.getMaxBlockSigops() + 1);
        }
        b16.solve();
        ...
        {
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSize() - b23.block.getMessageSize() - 65];
        ...
        b23.solve();
        checkState(b23.block.getMessageSize() == params.getMaxBlockSize());
        ...
        {
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSize() - b24.block.getMessageSize() - 64];
            Arrays.fill(outputScript, (byte) OP_FALSE);
            tx.addOutput(new TransactionOutput(params, tx, ZERO, outputScript));
        ...
        b24.solve();
        checkState(b24.block.getMessageSize() == params.getMaxBlockSize() + 1);
        ...
        NewBlock b31 = createNextBlock(b30, chainHeadHeight + 9, out8, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[(params.getMaxBlockSigops() - sigOps) / 20];
        ...
        NewBlock b32 = createNextBlock(b31, chainHeadHeight + 10, out9, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[(params.getMaxBlockSigops() - sigOps) / 20 + (params.getMaxBlockSigops() - sigOps) % 20 + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKMULTISIG);
            for (int i = 0; i < (params.getMaxBlockSigops() - sigOps) % 20; i++)
        ...
        NewBlock b39 = createNextBlock(b35, chainHeadHeight + 12, null, null);
        {
            ...
            while (b39.block.getMessageSize() < params.getMaxBlockSize()) {
                Transaction tx = new Transaction(params);
                ...
                if (b39.block.getMessageSize() + tx.getMessageSize() < params.getMaxBlockSize()) {
                    b39.addTransaction(tx);
                    b39numP2SHOutputs++;
                } else
                    break;
        ...
        NewBlock b40 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
        {
            ...
            int numTxes = (params.getMaxBlockSigops() - sigOps) / b39sigOpsPerOutput;
            ...
            Transaction tx = new Transaction(params);
            tx.addInput(new TransactionInput(params, tx, new byte[]{OP_1}, lastOutPoint));
            byte[] scriptPubKey = new byte[params.getMaxBlockSigops() - sigOps + 1];
        ...
        if (runBarelyExpensiveTests) {
            b41 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
            {
                ...
                int numTxes = (params.getMaxBlockSigops() - sigOps)
                        / b39sigOpsPerOutput;
                checkState(numTxes <= b39numP2SHOutputs);
                ...
                tx.addInput(new TransactionInput(params, tx,
                        new byte[]{OP_1}, lastOutPoint));
                byte[] scriptPubKey = new byte[params.getMaxBlockSigops() - sigOps];
                Arrays.fill(scriptPubKey, (byte) OP_CHECKSIG);
                tx.addOutput(new TransactionOutput(params, tx, ZERO, scriptPubKey));
                b41.addTransaction(tx);
            }
        ...
        NewBlock b64Original;
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSize() - b64Original.block.getMessageSize() - 65];
            Arrays.fill(outputScript, (byte) OP_FALSE);
            tx.addOutput(new TransactionOutput(params, tx, ZERO, outputScript));
            addOnlyInputToTransaction(tx, b64Original);
            b64Original.addTransaction(tx);
            b64Original.solve();
            checkState(b64Original.block.getMessageSize() == params.getMaxBlockSize());
        ...
        NewBlock b73 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSigops() - sigOps + (int) Script.MAX_SCRIPT_ELEMENT_SIZE + 1 + 5 + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an element that is too large, the CHECKSIGs after that push are still counted
            outputScript[params.getMaxBlockSigops() - sigOps] = OP_PUSHDATA4;
            Utils.uint32ToByteArrayLE(Script.MAX_SCRIPT_ELEMENT_SIZE + 1, outputScript, params.getMaxBlockSigops() - sigOps + 1);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
        ...
        NewBlock b74 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSigops() - sigOps + (int) Script.MAX_SCRIPT_ELEMENT_SIZE + 42];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an invalid element, all previous CHECKSIGs are counted
            outputScript[params.getMaxBlockSigops() - sigOps + 1] = OP_PUSHDATA4;
            outputScript[params.getMaxBlockSigops() - sigOps + 2] = (byte) 0xfe;
            outputScript[params.getMaxBlockSigops() - sigOps + 3] = (byte) 0xff;
            outputScript[params.getMaxBlockSigops() - sigOps + 4] = (byte) 0xff;
            outputScript[params.getMaxBlockSigops() - sigOps + 5] = (byte) 0xff;
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
        ...
        NewBlock b75 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            outputScript[params.getMaxBlockSigops() - sigOps] = OP_PUSHDATA4;
            outputScript[params.getMaxBlockSigops() - sigOps + 1] = (byte) 0xff;
            outputScript[params.getMaxBlockSigops() - sigOps + 2] = (byte) 0xff;
            outputScript[params.getMaxBlockSigops() - sigOps + 3] = (byte) 0xff;
            outputScript[params.getMaxBlockSigops() - sigOps + 4] = (byte) 0xff;
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
        ...
        NewBlock b76 = createNextBlock(b75, chainHeadHeight + 24, out23, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.getMaxBlockSigops() - sigOps + (int) Script.MAX_SCRIPT_ELEMENT_SIZE + 1 + 5];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an element that is filled with CHECKSIGs, they (obviously) arent counted
            outputScript[params.getMaxBlockSigops() - sigOps] = OP_PUSHDATA4;
            Utils.uint32ToByteArrayLE(params.getMaxBlockSigops(), outputScript, params.getMaxBlockSigops() - sigOps + 1);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
        ...
        int largeReorgLastHeight = chainHeadHeight + 33 + LARGE_REORG_SIZE + 1;
        {
            ...
            for (int i = 0; i < LARGE_REORG_SIZE; i++) {
                ...
                Transaction tx = new Transaction(params);
                byte[] outputScript = new byte[params.getMaxBlockSize() - nextBlock.block.getMessageSize() - 65];
                Arrays.fill(outputScript, (byte) OP_FALSE);
                tx.addOutput(new TransactionOutput(params, tx, ZERO, outputScript));
        ...
        if (runExpensiveTests) {
            ...
            for (blockCountAfter1001 = 0; blockCountAfter1001 < TRANSACTION_CREATION_BLOCKS; blockCountAfter1001++) {
                ...
                while (block.block.getMessageSize() < params.getMaxBlockSize() - 500) {
                    Transaction tx = new Transaction(params);
            ...
            for (int i = 0; hashes.hasNext(); i++) {
                NewBlock block = createNextBlock(lastBlock, nextHeight++, null, null);
                while (block.block.getMessageSize() < params.getMaxBlockSize() - 500 && hashes.hasNext()) {
                    Transaction tx = new Transaction(params);
